...
public class MultiLevelOverrideStrategy implements TurboFilterStrategy {
    ...
    private final Map<String, Level> overridingLogLevels = new ConcurrentHashMap<>();  // {partial-logger-name : level}
    private final Map<String, Integer> cachedLoggerLevels = new ConcurrentHashMap<>();  // {full-logger-name : intLevel}
    ...
    public FilterReply decide(Marker marker, Logger logger, Level level, String format, Object[] params, Throwable t) {
        ...
        final Integer intLevel = cacheLoggerNameMatch(loggerName);
        if (intLevel == NO_LEVEL) {
            return FilterReply.NEUTRAL;
        }
        final Level overridingLevel = Level.toLevel(intLevel);
